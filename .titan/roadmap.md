...
- **Phases 000–014 are complete; Phase 015 is in-progress.** No governance document requires a `blocked` status merely because milestone-level blockers exist. Phase 015's CI/pipeline-hygiene work (format-check, no-skipped-tests, basic secrets scan, dependency audit) is implemented and verified, but its remaining milestones (artifact packaging, staging environment, staging validation, production deployment, rollback) are governance-blocked pending a hosting/deployment-target decision (`tech_stack.md` §2/§5) and resolution or risk-acceptance (via ADR) of existing high/critical dependency vulnerabilities (`deployment_strategy.md` §7) — see `phases/phase-015-deployment-readiness.md`'s Governance Resolution. The dependency between Phase 002 and Phase 003 remains intentionally reversed from the original roadmap: architecture boundaries were approved before final stack selection so the stack could be evaluated against a concrete engine model. This reordering is recorded in ADR-0002.
