...
6. **AI/Agent interaction capability** - Titan Shell (Phase 018) is the control plane for the seven existing engines, not a home for AI/LLM interaction. No LLM client, provider dependency, or outbound AI call exists anywhere in this codebase today. When this capability is eventually built, its architectural home should be an **eighth engine** (e.g. `AgentEngine`), implementing the same `TitanEngine` lifecycle contract (`specification/engine_api.md` §3) as every other engine — registered in `EngineRegistry`, getting health/metadata/capability-discovery for free, addressable from the shell exactly like any other engine, no special-casing. Its "agent loop" (plan -> act -> observe) should *compose* the five engines that already implement those stages (Planner, Orchestrator, Execution, Validation, Learning) rather than reimplementing them; its genuinely novel surface is narrower: LLM provider abstraction, model selection, prompt templates, tool-calling, context injection, memory, and an AI-specific permission boundary. This requires its own ADR and phase — explicitly out of scope for Phase 018 (recorded per that phase's Milestone 5, 2026-08-08).
